...
const CATEGORIES = [
  'Comida','Repostería','Ropa','Tecnología','Belleza','Fotografía',
  'Carpintería','Electricidad','Automotriz','Hogar','Educación','Servicios','Otro',
]
  ...
  const onSubmit = async (data: PostFormValues) => {
    ...
    try {
      ...
      if (!postId) {
        ...
        await updatePost(postId, { ...data, tags: data.tags.split(',').map(t => t.trim()).filter(Boolean) } as any)
        toast.success('Publicación actualizada')
      }
      if (imageFiles.length && postId) {
        const urls = await uploadPostImages(postId, imageFiles, p => setProgress(p))
        await updatePost(postId, { images: urls } as any)
      }
