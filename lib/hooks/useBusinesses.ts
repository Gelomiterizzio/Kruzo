...
export function useBusinesses({ category, featured, zone, pageSize = 12 }: Options = {}) {
  const q = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => getBusinesses({ category, featured, zone, pageSize, cursor: pageParam }),
    initialPageParam: undefined as DocumentSnapshot | undefined,
    getNextPageParam: (last) => (last.businesses.length === pageSize ? (last.lastDoc ?? undefined) : undefined),
  })
  ...
  return {
    ...
    error: q.error ? (q.error instanceof Error ? q.error.message : 'Error al cargar negocios') : null,
